...
public class HeightmapConverter {
    ...
    public HeightmapConverter(int mapWidth, int mapHeight, int strength, String heightmapFile)
    {
        ...
        this.heightmapFile = "core/assets/island-tut4.jpg";
    ...
    {
        int pointer = 0;
        for(int gy=0;gy<vertices.length-1;gy++)
        {
            for(int gx=0;gx<vertices.length-1;gx++)
            {
                int topLeft = (gy*vertices.length)+gx;
                int topRight = topLeft + 1;
                int bottomLeft = ((gy+1)*vertices.length)+gx;
