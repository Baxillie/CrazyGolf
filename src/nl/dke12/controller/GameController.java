...
public class GameController
{
	...
	public void moveCamera(Camera camera)
	{
		...
		{
			camera.translate(-directVector.x,-directVector.y,0);
		}
		...
		{
			camera.translate(sideVector.x,sideVector.y,0);
		}
	...
	public void move()
	{
		...
		//Here are the controls for multiplayer
		
		if(multiplayer){

			if (Gdx.input.isKeyPressed(Input.Keys.NUM_2) && !player1Turn){
				
				pushBall2(new Vector3(shotVector.x, shotVector.y, 0));
				player1Turn = true;
			}

			if (Gdx.input.isKeyPressed(Input.Keys.NUM_1) && player1Turn) {
	...
	public static void trueMultiplayer(){
		multiplayer = true;
	}
